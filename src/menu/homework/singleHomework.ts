...
export default async function singleHomework(
	homework: homeworkType,
	isCurrentHomework: boolean
) {
	console.clear();
	console.log(
		chalk.yellow(figlet.textSync(functions.capitilise(homework.subject)))
	);

	let homeworkDetails: homeworkDetailsType = await edulink.homeworkDetails(
		homework.id
	);

	let attachments: attachmentType[] = [];
	for (const attachment of homework.attachments) {
		const attachmentDetails = await edulink.getAttachment(
			homework.id,
			attachment.id
		);
	...
	console.log(
		`${chalk.blue('Due Date:')} ${homework.due_date} (${homework.due_text})`
	);
	...
	if (attachments.length) {
		...
		for (const attachment of attachments) {
			console.log(
				functions.link(attachment.filename, attachment.url ?? '') ||
					`${chalk.cyan(`${attachment.filename}:`)} ${attachment.url}`
			);
		}
		console.log('\n');
	}
