...
export default {
	getHomework: async (): Promise<homeworkType[]> => {
	...
	homeworkDetails: async (
		id: number | string
	): Promise<homeworkDetailsType> => {
		return fetch(
			`${state.school.fromCode.server}?method=EduLink.HomeworkDetails`,
			{
				headers: {
					accept: 'application/json, text/plain, */*',
					'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8',
	...
	): Promise<attachmentFetchType> => {
		return fetch(
			`${state.school.fromCode.server}?method=EduLink.HomeworkAttachmentFetch`,
			{
				headers: {
					accept: 'application/json, text/plain, */*',
					'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8',
	...
	changeCompletionStatus: async (id: string | number, completed: boolean) => {
		return fetch(
			`${state.school.fromCode.server}?method=EduLink.HomeworkCompleted`,
			{
				headers: {
					accept: 'application/json, text/plain, */*',
					'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8',
